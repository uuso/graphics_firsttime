...
log1 = "C:/Users/service/git/pet_temperature_notifier/logs/log.txt"
logs_in_minute = 3
...
def makeplot_datetime_interpolation(filename, values_timestamp, values_y, compress=300):
	...
	lazy_intervals = {
		...
		4: 30}
	interval = lazy_intervals.get(int(len(values_timestamp) / (logs_in_minute*60)), 60) # calculate tick interval
...
def makeplot_datetime(filename, values_time, values_y):
	...
	interval = lazy_intervals.get(int(len(values_time) / (logs_in_minute*60)), 60) # calculate tick interval
...
if __name__ == "__main__":
	last_hours = 8
	time, temp = load_jsonfile_arglists(log1, "timestamp", "temperature", backwards=True, count=logs_in_minute*60*last_hours)
	filter_values(temp)
